...
class App extends Component {
  ...
  clearTimeInterval = () => {
    ...
    this.setState({time: '0', isCardActive: true})
  }

  categoryIdDetails = id => {
  ...
  changeImage = () => {
    ...
    this.setState({mainImageUrl: newImageUrl})
  }

  checkImage = imageUrl => {
    const {mainImageUrl, matchScore} = this.state
    if (mainImageUrl === imageUrl) {
      this.setState({matchScore: parseInt(matchScore) + 1})
      this.changeImage()
    } else {
      this.clearTimeInterval()
    }
  ...
  tick = () => {
    ...
    if (parsedTime <= 0) {
      ...
      this.setState({time: parsedTime - 1})
    }
  }
  ...
  render() {
    ...
    const finalCategoryList = imagesList.filter(
      eachImage => eachImage.category === categoryId,
    )
    return (
          ...
          <ul className="score-time-container">
            <li className="list-container">
              <p className="score-style">
                score: <span className="score-style1">{matchScore}</span>
              </p>
            </li>
            <li className="timer-image-container">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png "
                alt="timer"
                className="timer-image-style"
              />
              <p className="timer-style">{time} sec</p>
            </li>
          </ul>
        </nav>
          ...
          {!isCardActive ? (
            <div>
              <div className="sub-container">
                <img className="image-style" src={mainImageUrl} alt="match" />
              </div>
              ...
              <ul className="tab-list-container">
                {tabsList.map(eachTab => (
                  <TabItem
                    tabDetails={eachTab}
                    key={eachTab.tabId}
                    categoryIdDetails={this.categoryIdDetails}
                    isActive={eachTab.tabId === categoryId}
                  />
                ))}
              </ul>
              <ul className="images-list-container">
                {finalCategoryList.map(eachImage => (
                  <ImageItem
            ...
            <div className="score-container">
              <div className="trophy-container">
                ...
                />
                <p className="score-paragraph-style">YOUR SCORE</p>
                <h1 className="score-heading-style">{matchScore}</h1>
                <button
                  type="button"
                ...
                >
                  ...
                  />
                  <p className="play-paragraph-style">PLAY AGAIN</p>
                </button>
              </div>
            </div>
          )}
